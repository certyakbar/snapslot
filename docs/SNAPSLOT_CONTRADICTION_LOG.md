...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| C-001 | Constitution / Architecture / Phase Tasks | Acceptance Ledger | Password reset is required current-phase trust architecture, but proof is UNVERIFIED | Security/trust workflow is incomplete if claimed as done | RESOLVED: Password reset implemented and proven by commit `87910df` (PR #24). `tests/passwordReset.test.ts` PASS 3 tests on `origin/main`. Acceptance Ledger row updated to PASS. C-013 cross-reference remains for historical record only. | Auth workflow owner | Phase 1 | RESOLVED |
...
| C-012 | Constitution / Architecture / Phase Tasks | Acceptance Ledger / exact route proof | Platform owner registration/login is required as a surface, but exact route naming and full UI policy are not fully spelled out in the current matrix proof set | Owner workflow can drift between implementation and planning docs | Treat platform-owner auth as REAL AND PROVEN at workflow level where ledger proves it, but keep exact route/path normalization open. Session enforcement is now explicitly proven: tests/sessionBoundary.test.ts (PR #21 / T-1-A1, PASS 4 tests) confirms owner cookie cannot access business admin routes ( 401), business cookie cannot access owner admin routes ( 401), no-cookie access returns 401, and cross-business access returns 403. Route/path normalization for the platform owner surface remains open under Phase 9. PARTIALLY RESOLVED is the correct status. | Platform owner workflow owner | Phase 9 | PARTIALLY RESOLVED |
| C-013 | Constitution / Architecture | Acceptance Ledger | Password reset is described as trust requirement but also previously treated as later-phase / unverified in proof comments | Planning ambiguity can let contributors over- or under-scope the task | RESOLVED via C-001. Password reset is now fully implemented and proven: commit `87910df` (PR #24), `tests/passwordReset.test.ts` PASS 3 tests on `origin/main`. Historical planning ambiguity is closed. | Auth workflow owner | Phase 1 | RESOLVED |
