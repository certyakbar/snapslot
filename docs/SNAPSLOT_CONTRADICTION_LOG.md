...
| C-024 | Constitution Â§16 / Architecture Â§8.1 | code reality (server.ts `POST /api/snapslot-admin/login`) | Platform owner password is stored and compared as a plain environment variable string using `===`. No hashing, no salt, no constant-time comparison. If the `SNAPSLOT_ADMIN_PASSWORD` environment variable is leaked, the entire platform is compromised with no cryptographic protection. | Constitution Â§16 requires authenticated routes and password hashing. The business auth implementation uses scrypt with a salt and timing-safe comparison. The platform owner auth does not meet the same standard. | Treat as a REQUIRED SECURITY FIX in Phase 1. Hash the owner password with the same auth primitives as business auth. | Auth / platform owner owner | Phase 1 | OPEN |
